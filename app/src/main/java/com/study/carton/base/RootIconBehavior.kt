...
class RootIconBehavior : CoordinatorLayout.Behavior<ConstraintLayout> {
    ...
    private var layoutHeightDistance = 0
    private var layoutWidthDistance = 0
        ...
        if (dependency is AppBarLayout) {
            if (mTv1 == null) {
                ...
                mIv1?.post {
                    ...
                    layoutWidthDistance =
                        DisplayUtils.screenWidth - (rv1Position + mLayout1!!.right - defaultMargin - imageWidth)
    ...
    private fun getImageViewByEnd(layout: View?, textView: TextView?): Int {
        return DisplayUtils.screenWidth - layout!!.right + textView!!.width
    }
    ...
    override fun onDependentViewChanged(
        ...
        if (dependency is AppBarLayout) {
            ...
            if (layoutWidthDistance != 0) {
                ...
                val dy = kotlin.math.abs(ArithHelper.mul(dependency.y, heightRatio))

                if (rv1Position != 0) {
                    val ro = ArithHelper.div(rv1Position.toFloat(), totalScrollRange)
                    val tx = kotlin.math.abs(ArithHelper.mul(dependency.y, ro))
                ...
                val absY = kotlin.math.abs(dependency.y)

                val dw = kotlin.math.abs(ArithHelper.mul(dependency.y, widthRatio))
